...
DB_PATH = Path(__file__).parent.parent / "speedreader.db"
    ...
    with get_connection() as conn:
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                source_type TEXT NOT NULL,          -- pdf, txt, url, epub
            ...
            CREATE TABLE IF NOT EXISTS progress (
                ...
                mode          TEXT NOT NULL,         -- bionic, rsvp
                word_position INTEGER DEFAULT 0,     -- RSVP için kelime indeksi
                scroll_pct    REAL DEFAULT 0.0,      -- Bionic için scroll %
            ...
            CREATE TABLE IF NOT EXISTS reading_stats (
                ...
                duration_sec INTEGER DEFAULT 0,      -- kaç saniye okuma yapıldı
...
def create_session(source: str, source_type: str, total_words: int) -> int:
    """Yeni okuma oturumu oluştur, session_id döner."""
    with get_connection() as conn:
        cur = conn.execute(
...
def save_progress(session_id: int, mode: str, word_position: int = 0,
    ...
    """Mevcut okuma pozisyonunu kaydet (UPSERT mantığı)."""
    ...
    with get_connection() as conn:
        ...
        if existing:
            conn.execute(
                """UPDATE progress 
                   SET word_position=?, scroll_pct=?, wpm=?, updated_at=?
            ...
            conn.execute(
                """INSERT INTO progress 
                   (session_id, mode, word_position, scroll_pct, wpm, updated_at)
                   VALUES (?,?,?,?,?,?)""",
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO reading_stats 
               (session_id, mode, words_read, duration_sec, avg_wpm, completed, finished_at)
               VALUES (?,?,?,?,?,?,?)""",
...
def get_all_stats() -> list[dict]:
    """Tüm okuma istatistiklerini getir (dashboard için)."""
    with get_connection() as conn:
        rows = conn.execute("""
            SELECT 
                s.source, s.source_type, s.total_words, s.created_at,
...
def get_dashboard_summary() -> dict:
    ...
    with get_connection() as conn:
        ...
        # Son 7 günlük günlük okuma (kelime sayısı)
        daily = conn.execute("""
            SELECT
...
def get_session_with_progress(session_id: int) -> dict | None:
    """Session + ilerleme bilgisini birleştirerek getir."""
    with get_connection() as conn:
        row = conn.execute("""
